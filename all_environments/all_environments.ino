...
void setup() {
  ...
  Serial.println("3 = Wall with Damping");
  Serial.println("4 = Damper");
  Serial.println("5 = Nonlinear Friction");
  Serial.println("6 = Bump");
  Serial.println("7 = Better Bump");
  Serial.println("8 = Bump and Valley");
  Serial.println("9 = Hard Surface");
  Serial.println("A = Mass Spring Damper");
}
...
void loop() {
  ...
  switch (mode) {
    ...
    case '7':
    {
      ...
      if ((xh > bump_location - bump_width / 2.0) &&
          (xh < bump_location)) {

        motorCommand = -bump_force;
      }
      else if ((xh >= bump_location) &&
               (xh < bump_location + bump_width / 2.0)) {
    ...
    case '8':
    {
      ...
      if ((xh <= (bump_location + bump_length / 2.0)) &&
          (xh >= (bump_location - bump_length / 2.0))) {

        motorCommand =
          bump_k * bump_height *
          cos(PI / bump_length * (xh - bump_location)) *
          sin(PI / bump_length * (xh - bump_location));
      }
      else if ((xh <= (valley_location + valley_length / 2.0)) &&
               (xh >= (valley_location - valley_length / 2.0))) {

        motorCommand =
          -valley_k * valley_height *
          cos(PI / valley_length * (xh - valley_location)) *
          sin(PI / valley_length * (xh - valley_location));
      }
      else {
